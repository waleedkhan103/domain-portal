...
if ($stmt = $conn->prepare("SELECT COUNT(*) as cnt FROM domains WHERE expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY)")) {
  $stmt->execute();
  $r = $stmt->get_result();
  $expiring30 = ($r->fetch_assoc()['cnt'] ?? 0);
  $stmt->close();
} else {
  $r = mysqli_query($conn, "SELECT COUNT(*) as cnt FROM domains WHERE expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY)");
...
$revenueThis = 0;
$revenueLast = 0;
if ($stmt = $conn->prepare("SELECT IFNULL(SUM(total_amount),0) as s FROM orders WHERE created_at >= DATE_FORMAT(NOW(), '%Y-%m-01')")) {
  $stmt->execute();
  $r = $stmt->get_result();
  $revenueThis = (float) ($r->fetch_assoc()['s'] ?? 0);
  $stmt->close();
} else {
  $r = mysqli_query($conn, "SELECT IFNULL(SUM(total_amount),0) as s FROM orders WHERE created_at >= DATE_FORMAT(NOW(), '%Y-%m-01')");
  $revenueThis = ($r ? (float) mysqli_fetch_assoc($r)['s'] : 0);
}
if ($stmt = $conn->prepare("SELECT IFNULL(SUM(total_amount),0) as s FROM orders WHERE created_at >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 MONTH),'%Y-%m-01') AND created_at < DATE_FORMAT(NOW(), '%Y-%m-01')")) {
  $stmt->execute();
  $r = $stmt->get_result();
  $revenueLast = (float) ($r->fetch_assoc()['s'] ?? 0);
  $stmt->close();
} else {
  $r = mysqli_query($conn, "SELECT IFNULL(SUM(total_amount),0) as s FROM orders WHERE created_at >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 MONTH),'%Y-%m-01') AND created_at < DATE_FORMAT(NOW(), '%Y-%m-01')");
...
$recentRegistrations = 0;
if ($stmt = $conn->prepare("SELECT COUNT(*) as cnt FROM domains WHERE registration_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)")) {
  $stmt->execute();
  $r = $stmt->get_result();
  $recentRegistrations = ($r->fetch_assoc()['cnt'] ?? 0);
  $stmt->close();
} else {
  $r = mysqli_query($conn, "SELECT COUNT(*) as cnt FROM domains WHERE registration_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)");
  $recentRegistrations = ($r ? mysqli_fetch_assoc($r)['cnt'] : 0);
}
...
$recentDomains = [];
$sqlRecentDomains = "SELECT d.id, d.domain_name, d.user_id, d.status, d.registration_date as registered_at, u.email FROM domains d LEFT JOIN users u ON u.id = d.user_id ORDER BY d.registration_date DESC LIMIT 10";
...
$recentOrders = [];
if ($stmt = $conn->prepare("SELECT o.id, o.order_number, o.user_id, o.total_amount as total, o.status, o.created_at, u.email FROM orders o LEFT JOIN users u ON u.id = o.user_id ORDER BY o.id DESC LIMIT 10")) {
  $stmt->execute();
  $r = $stmt->get_result();
  while ($row = $r->fetch_assoc())
    $recentOrders[] = $row;
  ...
  $r = mysqli_query($conn, "SELECT o.id, o.order_number, o.user_id, o.total_amount as total, o.status, o.created_at, u.email FROM orders o LEFT JOIN users u ON u.id = o.user_id ORDER BY o.id DESC LIMIT 10");
  if ($r) {
    while ($row = mysqli_fetch_assoc($r))
      $recentOrders[] = $row;
  }
...
// Alerts
$alerts = [];
if ($stmt = $conn->prepare("SELECT COUNT(*) as cnt FROM domains WHERE expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 7 DAY)")) {
  $stmt->execute();
  $r = $stmt->get_result();
  $alerts['expiring_7'] = (int) ($r->fetch_assoc()['cnt'] ?? 0);
  $stmt->close();
} else {
  $r = mysqli_query($conn, "SELECT COUNT(*) as cnt FROM domains WHERE expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 7 DAY)");
  $alerts['expiring_7'] = ($r ? (int) mysqli_fetch_assoc($r)['cnt'] : 0);
}
if ($stmt = $conn->prepare("SELECT COUNT(*) as cnt FROM domains WHERE expiry_date BETWEEN DATE_ADD(CURDATE(), INTERVAL 8 DAY) AND DATE_ADD(CURDATE(), INTERVAL 14 DAY)")) {
  $stmt->execute();
  $r = $stmt->get_result();
  $alerts['expiring_14'] = (int) ($r->fetch_assoc()['cnt'] ?? 0);
  $stmt->close();
} else {
  $r = mysqli_query($conn, "SELECT COUNT(*) as cnt FROM domains WHERE expiry_date BETWEEN DATE_ADD(CURDATE(), INTERVAL 8 DAY) AND DATE_ADD(CURDATE(), INTERVAL 14 DAY)");
